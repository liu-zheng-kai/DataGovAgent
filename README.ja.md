...
本プロジェクトは [`AGPL-3.0-or-later`](./LICENSE) で公開されています。

- 個人または組織が本プロジェクトを改変して再配布する場合、改変版も同じライセンスでソース公開する必要があります。
- 改変版をネットワーク経由でユーザーに提供する場合も、対応する完全なソースコードを提供する必要があります。
- リポジトリ方針として、外部向けに再利用または改変した場合は、改変後のソースを公開 Git リポジトリに公開することを求めます。GitHub を推奨します。詳しくは [OPEN_SOURCE_POLICY.md](./OPEN_SOURCE_POLICY.md) を参照してください。
